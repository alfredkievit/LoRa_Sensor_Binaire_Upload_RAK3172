...
5. Build en upload de sketch `RAK3172_Binaire_payload_rui.ino`.

Secrets:
- `SECRET_DEV_EUI` hoort overeen te komen met de DevEUI van het module-sticker of de eerder werkende firmware.
- Werk met `arduino_secrets.h` voor echte OTAA-sleutels.
- Gebruik `arduino_secrets.example.h` als template voor nieuwe deployments.
